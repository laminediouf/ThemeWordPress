...
function heropress_css() {
	...
	wp_dequeue_style('specia-menu', get_template_directory_uri() . '/css/menus.css');
}
...
function heropress_remove_parent_setting( $wp_customize ) {
	...
	$wp_customize->remove_control('instagram_link');
...
function heropress_add_to_cart_fragment( $fragments ) {
    ...
    ?><a class="cart-icon" href="<?php echo WC()->cart->get_cart_url(); ?>"><i class='fa fa-cart-arrow-down'></i><?php
    if ( $count > 0 ) { 
	?>
 ...
 */
require( get_stylesheet_directory() . '/inc/demo-import/index.php');

/**
 * Called Required Plugin Features
 */
require( get_stylesheet_directory() . '/inc/required-plugin/index.php');